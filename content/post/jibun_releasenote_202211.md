...
今月は外出が多かったため読書する時間があまり取れませんでした。定期的に図書館に行く習慣ができていること、読みたいと思っていた技術以外の本が読めたことはよかったです。また、月末にあった社内イベントで『Go言語でつくるインタプリタ』をOcamlで実装した話を聞いて奮起し、ずっと途中になっていたのを読み終えることができました。
...
OuraRingで平均歩数の計算がうまくできなかったので、自動で出力するスクリプトを書きました。
...
- 今年度の目標は残すところ一つ、無事に終わりそう
  ...
  - その前にラスト一つの積み残しをやる
- 社内LT大会に出た
  - 10月に続きWordPressネタ（ウケると思ったので）にしたが、納得できなかった（もっとテックな内容にしたかった）ので次回はそうする
